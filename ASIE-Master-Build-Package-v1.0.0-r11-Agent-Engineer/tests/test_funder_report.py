...
class FunderReportProjectionTests(unittest.TestCase):
    ...
    def test_pdf_export_is_server_side_and_snapshot_bound(self) -> None:
        import os, shutil
        if not (os.environ.get("ASIE_PDF_RENDERER") or shutil.which("chrome") or shutil.which("msedge")):
            self.skipTest("PDF renderer is supplied by the document export environment")
        repo = self.make_repo()
