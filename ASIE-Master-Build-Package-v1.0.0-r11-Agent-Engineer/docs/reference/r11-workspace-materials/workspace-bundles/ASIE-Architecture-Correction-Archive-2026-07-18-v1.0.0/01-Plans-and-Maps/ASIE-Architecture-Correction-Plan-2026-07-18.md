...
ما بُني حتى الآن ليس واجهات فقط؛ لدينا نواة منتج محلية حقيقية في مجالات Finance, Evidence, Source Governance, Sector Intelligence, Risk, Execution, Decision Pack, Snapshot. لكنها بُنيت كـ **modular local application** وليس كتطبيق كامل لمعمارية AAS التشغيلية المجمدة.
...
| Module Lifecycle | discovery/registration/validation/binding/activation/isolation | `backend/module_runtime.py` يشغل handler مسجل بعد Bus/Socket | جزئي v7.1؛ Finance وEvidence Ledger وSector Intelligence وDecision Council وRisk Engine وExecution Engine وReport Module داخل `build_overview()` انتقلت، وExecution يقرأ Risk عبر summary contract مغلق فقط |
| Message Flow | messages with source/target/contract/socket/correlation/audit | `BusMessage` يفرض envelope تشغيليًا | جزئي v7.1؛ Finance وEvidence Ledger وSector Intelligence وDecision Council وRisk Engine وExecution Engine وReport Module ضمن مسار المنتج تستخدم الرسائل، مع `risk.advisory.summary.v1` بين Risk وExecution |
...
- `build_overview()` now creates one `RunScopedModuleRuntime` for Finance, Evidence, Sector, Decision, Risk, and Execution.
