...
لا يقبل العميل توسيع `include_domains` أو `exclude_domains`، ولا يقبل Tavily بذرة URL حرة. في `crawl/map` يشتق الخادم `select_domains` و`select_paths` من المضيف وجذور `allowed_paths` المعتمدة، ويحوّل أي تضييق يطلبه العميل من مسار حرفي إلى regex آمن؛ لا تُمرر regexات العميل إلى المزود. المصادر `candidate` و`reference_only` و`blocked` لا يسمح لها بالاستخراج أو الزحف. ينطبق ذلك صراحة على `MOSTAQL_PROJECTS`.
...
- رفض توسيع مسارات `crawl/map` ورفض المضيفات الداخلية وIP literals والمسارات الملتبسة قبل النقل.
- إثبات إرسال فلاتر المجال والمسار المشتقة من الخادم إلى `crawl/map`.
